...
static SWsimdResult
sw_simd_uint32([[maybe_unused]] const uint8_t *original_dbseq,
               ...
               SSWresources *ssw_resources)
{
  ...
  const simd_int vGapO = simdi32_set(weight_gapO);
  const simd_int vGapE = simdi32_set(weight_gapE);
...
static_cast<uint32_t>(abs_smallest_score)
      ...
      < UINT32_MAX)
  {
    /* Trace the column with the max alignment score for the ending
       position on query. */
    const uint32_t *ptr     = reinterpret_cast<uint32_t *>(pvHmax);
