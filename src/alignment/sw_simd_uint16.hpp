/* generated by ./ssw_gen.py 16 DO NOT EDIT */
#ifndef SW_SIMD_UINT16_HPP
#define SW_SIMD_UINT16_HPP
...
static SWsimdResult sw_simd_uint16(GTTL_UNUSED const uint8_t *original_dbseq,
                                   ...
                                   SSWresources *ssw_resources)
{
  ...
  } else
  {
    ...
    ssw_resources->reset16(segment_len);
    own_resources = false;
  }
  simd_int *pvHStore = ssw_resources->vectors16_get();
...
#define SSW_MAX_CMP_VALUE UINT32_MAX
    ...
    for (size_t k = 0; GTTL_IS_LIKELY(k < simd_size); ++k)
    {
           ...
           ++segment_pos)
      {
        ...
        vF = simdui16_subs(vF, vGapE);
        if (GTTL_IS_UNLIKELY(!simdi8_movemask(simdi16_gt(vF, vH))))
        {
          break;
        }
      }
    ...
    {
      const uint16_t local_max_score = simdi16_hmax(vMaxScore);
    ...
    if (expected_score > 0 && simdi16_hmax(vMaxColumn) == expected_score)
    {
      break;
    }
    dbseq_pos += step;
#if SSW_SIMD_DEBUG > 1
    uint16_t *ptr = reinterpret_cast<uint16_t *>(pvHStore);
      ...
      UINT16_MAX)
  {
    /* Trace the column with the max alignment score for the ending
       position on query. */
    uint16_t *ptr = reinterpret_cast<uint16_t *>(pvHmax);
